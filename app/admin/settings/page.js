...
export default function SystemSettings() {
  ...
  const handleSaveSettings = async (category, data) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/admin/settings/${routeName}`, {
        method: 'PUT',
        headers: {
  ...
  const handleToggleMaintenanceMode = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/admin/settings/maintenance/toggle', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleExportSettings = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/admin/settings/export', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
