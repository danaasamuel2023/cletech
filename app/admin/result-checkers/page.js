// app/admin/result-checkers/page.js - Fixed Input Handling
'use client';
...
// Move BulkUploadModal outside and make it a separate component
const BulkUploadModal = ({ isOpen, onClose, onUpload }) => {
  // Use local state for all form inputs
  const [localUploadData, setLocalUploadData] = useState({
    type: 'WASSCE',
  ...
  const handleSubmit = () => {
    // Parse bulk input
    const lines = localBulkInput.trim().split('\n');
      ...
      <div className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <select
              value={localUploadData.type}
              onChange={(e) => setLocalUploadData({...localUploadData, type: e.target.value})}
          ...
          <div>
            ...
            <input
              type="number"
              value={localUploadData.year}
              onChange={(e) => setLocalUploadData({...localUploadData, year: parseInt(e.target.value)})}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
              min="2020"
              max="2030"
            />
          ...
          <div>
            ...
            <select
              value={localUploadData.examType}
              onChange={(e) => setLocalUploadData({...localUploadData, examType: e.target.value})}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
          ...
          <div>
            ...
            <input
              type="number"
              value={localUploadData.price}
              onChange={(e) => setLocalUploadData({...localUploadData, price: parseFloat(e.target.value)})}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
              min="0"
              step="0.5"
            />
        ...
        <div className="mb-4">
          ...
          <textarea
            value={localBulkInput}
            onChange={(e) => setLocalBulkInput(e.target.value)}
...
WAS24001235,DEF456UVW789"
          ...
          <div className="mt-2 flex items-start space-x-2">
            ...
            <p className="text-sm text-gray-600">
              ...
              {localBulkInput && ` ${localBulkInput.trim().split('\n').filter(l => l.trim()).length} cards detected.`}
            </p>
          </div>
        </div>
...
// CheckerDetailsModal as a separate component
const CheckerDetailsModal = ({ isOpen, onClose, checker }) => {
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-6 text-white">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-2xl font-bold font-mono">{checker.serialNumber}</p>
              </div>
              <div className="text-right">
                <p className="text-purple-100 text-sm">PIN</p>
                <p className="text-xl font-bold font-mono">{checker.pin}</p>
              </div>
            </div>
            <div className="mt-4 flex justify-between items-center">
              <div>
                <p className="text-purple-100 text-sm">{checker.type} • {checker.year}</p>
                <p className="text-white font-medium">{checker.examType}</p>
              </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="font-semibold text-gray-900">GHS {checker.price}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-sm text-gray-500">Batch Number</p>
              <p className="font-semibold text-gray-900">{checker.batchInfo.batchNumber}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-sm text-gray-500">Added By</p>
              <p className="font-semibold text-gray-900">{checker.addedBy.name}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
                ...
                {checker.soldTo && (
                  <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">Sold to {checker.soldTo.user.name}</p>
                        <p className="text-xs text-gray-500">
                          {new Date(checker.soldTo.soldAt).toLocaleString()}
                        </p>
                      </div>
                    </div>
                    <p className="font-semibold">GHS {checker.soldTo.purchasePrice}</p>
                  </div>
                )}
                {checker.usedBy && (
                  <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">Used by {checker.usedBy.user.name}</p>
                        <p className="text-xs text-gray-500">
                          {new Date(checker.usedBy.usedAt).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  </div>
...
export default function ResultCheckersManagement() {
  ...
  const handleBulkUpload = useCallback(async (uploadPayload) => {
    try {
      const token = localStorage.getItem('token');
