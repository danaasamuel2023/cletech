...
const PaystackDeposit = () => {
  ...
  const fetchPaymentConfig = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/deposites/config', {
        headers: {
  ...
  const fetchRecentDeposits = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/deposits/history?limit=5', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const initializePayment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/deposites/initialize', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
